...
#************** DATOS ZABBIX PROXY *******************
ZABBIX_SERVER_IP_1="190.111.153.100"
ZABBIX_SERVER_IP_2="190.111.153.99"
...
for SERVER in "${SERVERS[@]}"; do
    if nc -zv "$SERVER" 10051 2>/dev/null; then
        log "Conectividad exitosa a $SERVER en el puerto 10051"
        CONNECTED_SERVER="$SERVER"
        break  # Salir del bucle al encontrar el primer servidor funcional
    else
        log "Advertencia: No se puede conectar a $SERVER en el puerto 10051"
    fi
done

if [ -z "$CONNECTED_SERVER" ]; then
    log "Error: No hay conectividad con ningún servidor Zabbix en el puerto 10051"
    exit 1
fi

# Aquí puedes usar $CONNECTED_SERVER para saber qué servidor está disponible
log "Usando servidor Zabbix: $CONNECTED_SERVER"
...
function Install_zabbix_agent() {
  log "=============================="
  log "Instalando Zabbix Agent $ZABBIX_AGENT_VERSION en Ubuntu 24.04..."
  log "=============================="
 ...
 #se agrego esta linea si el operador no actualizo previamente su servidor y solo ejecuto el script.
  apt update
  wget https://repo.zabbix.com/zabbix/6.0/ubuntu/pool/main/z/zabbix-release/zabbix-release_latest_6.0+ubuntu24.04_all.deb
  dpkg -i zabbix-release_latest_6.0+ubuntu24.04_all.deb
  apt update
 # Eliminar el archivo .deb
  rm -- "zabbix-release_latest_6.0+ubuntu24.04_all.deb"
...
# Borrar el script solo si todo salió bien
rm -- "$0"
